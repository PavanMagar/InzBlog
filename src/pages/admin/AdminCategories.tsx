    ...
    <div className="flex min-h-screen">
      ...
      <div className="flex-1 overflow-auto bg-background">
        <div className="border-b border-border bg-card px-4 py-5 sm:px-8 sm:py-6 lg:pl-8 pl-16">
          <h1 className="font-display text-xl font-bold text-foreground sm:text-2xl">Categories</h1>
        ...
        <div className="p-4 sm:p-8">
          ...
          {loading ? (
            ...
            <div className="py-10 text-center">
              <i className="fa-solid fa-folder-open mb-3 text-3xl text-muted-foreground/30"></i>
              <p className="text-muted-foreground">No categories yet.</p>
            </div>
          ) : (
            <div className="space-y-2">
              {categories.map((cat) => (
                <div key={cat.id} className="flex items-center justify-between rounded-2xl border border-border bg-card px-4 py-3 shadow-[var(--shadow-card)] sm:px-5">
                  {editId === cat.id ? (
                    <div className="flex flex-1 items-center gap-2">
                      <input
                        ...
                        autoFocus
                      />
                      <button onClick={updateCategory} className="rounded-lg p-1.5 text-primary hover:bg-muted"><Check className="h-4 w-4" /></button>
                      <button onClick={() => setEditId(null)} className="rounded-lg p-1.5 text-muted-foreground hover:bg-muted"><X className="h-4 w-4" /></button>
                    </div>
                  ) : (
                    <>
                      <div className="min-w-0 flex-1">
                        <span className="text-sm font-medium text-card-foreground">{cat.name}</span>
                        <span className="ml-2 hidden text-xs text-muted-foreground sm:inline">/{cat.slug}</span>
                      </div>
