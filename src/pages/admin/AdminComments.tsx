...
import { MessageCircle, Search, Send, Reply, Trash2, Eye, ChevronDown, ChevronUp, Filter } from "lucide-react";
...
export default function AdminComments() {
  ...
  const totalComments = comments.filter((c) => !c.parent_id && !c.is_admin_reply).length;
  const totalReplies = comments.filter((c) => c.parent_id).length;
  const totalAdminReplies = comments.filter((c) => c.is_admin_reply).length;
  const totalLikes = comments.reduce((sum, c) => sum + c.likes_count, 0);

  const statCards = [
    { label: "Comments", value: totalComments, color: "hsl(217, 91%, 60%)" },
    { label: "Replies", value: totalReplies, color: "hsl(271, 81%, 56%)" },
    { label: "Admin Replies", value: totalAdminReplies, color: "hsl(160, 60%, 48%)" },
    { label: "Total Likes", value: totalLikes, color: "hsl(0, 72%, 51%)" },
  ];
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((comment) => (
                  ...
                  >
                      ...
                      <div className="flex items-start gap-3 flex-1 min-w-0">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            {comment.parent_id && (
                              <span className="rounded-full bg-accent/10 px-2 py-0.5 text-[10px] font-medium text-accent">Reply</span>
                            )}
                          </div>
                          <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                            ...
                            {comment.likes_count > 0 && (
                              <>
                                <span>•</span>
                                <span className="text-red-500">♥ {comment.likes_count}</span>
                              </>
                            )}
                          </div>
                    ...
                    {/* Comment content */}
                    <div className="mt-3">
                      {comment.content.length > 200 && expandedComment !== comment.id ? (
                        <>
                          <p className="text-sm text-foreground/90">{comment.content.substring(0, 200)}...</p>
                          <button onClick={() => setExpandedComment(comment.id)} className="mt-1 flex items-center gap-1 text-xs text-primary hover:underline">
                            <Eye className="h-3 w-3" /> Read full comment
                          </button>
                        </>
                      ) : (
                        <>
                          ...
                          {comment.content.length > 200 && (
                            <button onClick={() => setExpandedComment(null)} className="mt-1 flex items-center gap-1 text-xs text-primary hover:underline">
