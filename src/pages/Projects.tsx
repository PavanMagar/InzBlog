      ...
      <main className="container px-4 py-8 pt-24 sm:px-6 md:py-12 md:pt-28">
        <div className="mb-10 text-center">
          ...
          <p className="mx-auto mt-3 max-w-lg text-sm text-muted-foreground">
            Featured projects, demos, and real-world implementations
          </p>
        </div>
                ...
                <div className="rounded-xl border border-border bg-background p-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <button
                      ...
                      className={`rounded-full px-4 py-2 text-xs font-medium transition-all ${!categorySlug ? "text-primary-foreground shadow-sm" : "border border-border bg-background text-foreground hover:bg-muted"}`}
                    ...
                    {(showAllCategories ? categories : categories.slice(0, VISIBLE_CATEGORIES)).map((c) => (
                      <button
                        ...
                        className={`rounded-full px-4 py-2 text-xs font-medium transition-all ${c.slug === categorySlug ? "text-primary-foreground shadow-sm" : "border border-border bg-background text-foreground hover:bg-muted"}`}
                  ...
                  {categories.length > VISIBLE_CATEGORIES && (
                    <button onClick={() => setShowAllCategories(!showAllCategories)} className="mt-3 inline-flex items-center gap-1 text-xs font-medium text-primary transition-colors hover:text-primary/80">
                      {showAllCategories ? <><ChevronUp className="h-3.5 w-3.5" /> Show less</> : <><ChevronDown className="h-3.5 w-3.5" /> Show {categories.length - VISIBLE_CATEGORIES} more</>}
                    </button>
                  )}
                </div>
              </motion.div>
        ...
        {loading ? (
          ...
          <>
            ...
            {hasMore && (
              <div className="mt-10 flex justify-center">
                ...
                >
                  <ChevronDown className="h-4 w-4" /> Load More ({allPosts.length - visibleCount} remaining)
                </button>
              </div>
            )}
          </>
          ...
          <div className="py-20 text-center">
            <i className="fa-solid fa-diagram-project mb-4 text-4xl text-muted-foreground/30"></i>
